...
class Funcs():
    ...
    def Concluir(self):
        ...
        for item in self.lista_car.get_children():
            valores = self.lista_car.item(item, 'values')
            cod = int(valores[0])
            nome = str(valores[1])
            quant = int(valores[2])
            valor = float(valores[3])
            valor_total = float(valores[3]) * float(valores[2])
            tipo = str(valores[4])

            self.conectar_bd()
            self.cursor.execute(
                "INSERT INTO Vendas (CPF_cliente,Valor_da_venda, Data,Tipo,Valor_produto, Nome_produto, Quantidade_venda,codigo_produto) VALUES(?,?,?,?,?,?,?,?)", (cpf, valor_total, data, tipo, valor, nome, quant, cod))
            self.conn.commit()
            self.desconecta_bd()
        self.troco.destroy()
    ...
    def busca_data(self):
        if self.data_relatorio.get() == "" and self.pesquisa_tipo.get() == "" and self.pesquisa_cpf.get() == "":
            self.conectar_bd()
            self.lista_relatorio_venda.delete(
                *self.lista_relatorio_venda.get_children())
            self.pesquisa_cod.insert(END, '%')
            cod = self.pesquisa_cod.get()
            self.cursor.execute(
                """ SELECT codigo_venda, CPF_cliente, Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda, valor_da_venda, Tipo FROM Vendas WHERE codigo_produto LIKE '%s'   """ % cod)
            busca_data = self.cursor.fetchall()

            for i in busca_data:
                self.lista_relatorio_venda.insert("", END, values=i)
            self.pesquisa_cod.delete(0, END)
            contador = 0
            for item in self.lista_relatorio_venda.get_children():
                valores = self.lista_relatorio_venda.item(item, 'values')
                valor = float(valores[7])
                contador = contador + valor

            self.ltotal_relatorio['text'] = " R${:.2f}" .format(contador)

            self.desconecta_bd()

        elif self.pesquisa_cod.get() == "" and self.data_relatorio.get() == "" and self.pesquisa_cpf.get() == "":
            self.conectar_bd()
            self.lista_relatorio_venda.delete(
                *self.lista_relatorio_venda.get_children())
            self.pesquisa_tipo.insert(END, '%')
            tipo = self.pesquisa_tipo.get()
            self.cursor.execute(
                """ SELECT codigo_venda, CPF_cliente, Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda,valor_da_venda,  Tipo FROM Vendas WHERE Tipo LIKE '%s'   """ % tipo)
            busca_data = self.cursor.fetchall()
            for i in busca_data:
                self.lista_relatorio_venda.insert("", END, values=i)
            self.pesquisa_tipo.delete(0, END)
            contador = 0
            for item in self.lista_relatorio_venda.get_children():
                valores = self.lista_relatorio_venda.item(item, 'values')
                valor = float(valores[7])
        ...
        elif self.pesquisa_cod.get() == "" and self.data_relatorio.get() == "" and self.pesquisa_tipo.get() == "":
            self.conectar_bd()
            self.lista_relatorio_venda.delete(
                *self.lista_relatorio_venda.get_children())
            self.pesquisa_cpf.insert(END, '%')
            cpf = self.pesquisa_cpf.get()
            self.cursor.execute(
                """ SELECT codigo_venda, CPF_cliente, Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda,valor_da_venda,  Tipo FROM Vendas WHERE CPF_cliente LIKE '%s'   """ % cpf)
            busca_data = self.cursor.fetchall()
            for i in busca_data:
                self.lista_relatorio_venda.insert("", END, values=i)
            self.pesquisa_cpf.delete(0, END)
            contador = 0
            for item in self.lista_relatorio_venda.get_children():
                valores = self.lista_relatorio_venda.item(item, 'values')
                valor = float(valores[7])
        ...
        elif self.data_relatorio.get() != "" and self.data_relatorio_final.get() == "" and self.pesquisa_cod.get() == "" and self.pesquisa_tipo.get() == "" and self.pesquisa_cpf.get() == "":
            self.conectar_bd()
            self.lista_relatorio_venda.delete(
                *self.lista_relatorio_venda.get_children())
            self.data_relatorio.insert(END, '%')
            data_inicial = self.data_relatorio.get()
            self.cursor.execute(
                """ SELECT codigo_venda, CPF_cliente, Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda, valor_da_venda, Tipo FROM Vendas WHERE Data like '{}'  """ .format(
                    data_inicial))
            busca_data = self.cursor.fetchall()
            for i in busca_data:
                self.lista_relatorio_venda.insert("", END, values=i)
            ...
            self.data_relatorio.insert(END, '%')
            data_inicial = self.data_relatorio.get()
            data_final = self.data_relatorio_final.get()
            self.cursor.execute(
                """ SELECT codigo_venda, CPF_cliente, Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda, valor_da_venda, Tipo FROM Vendas WHERE Data between '{}' and '{}'  """ .format(data_inicial, data_final))
            busca_data = self.cursor.fetchall()
            for i in busca_data:
                self.lista_relatorio_venda.insert("", END, values=i)
            self.data_relatorio.delete(0, END)
            self.data_relatorio_final.delete(0, END)
            contador = 0
            for item in self.lista_relatorio_venda.get_children():
                valores = self.lista_relatorio_venda.item(item, 'values')
                valor = float(valores[7])
    ...
    def remover_do_carrinho(self):
        ...
        quantida_valores = float(valores[2])
...
class sistema(Funcs):
    ...
    def conectar_bd(self):
        self.conn = sqlite3.connect("Xeroz4.db")
    ...
    def criarTabelas(self):
        ...
        CREATE TABLE IF NOT EXISTS Vendas(
            ...
            codigo_produto codigo_produto INTEGER,
            Nome_produto VARCHAR(100) NOT NULL,
            Valor_produto DECIMAL(10,2) NOT NULL,
    ...
    def select_lista_carrinho(self):
        ...
        if self.valor_produto_venda.get() != "" and self.quant.get() != "":
            self.lista_car.insert(parent="", index='end', text="", values=(self.codigo_produto_venda.get(), self.Nome_produto_venda.get(), self.quant.get(
            ), self.valor_produto_venda.get(), self.tipo_entry.get()))
    ...
    def lista_relatorio(self):
        ...
        relatorio = self.cursor.execute(
            """ SELECT codigo_venda,CPF_cliente,Data,codigo_produto,Nome_produto,Valor_produto,Quantidade_venda,  valor_da_venda, Tipo FROM Vendas ORDER BY codigo_venda ASC;""")
        for i in relatorio:
            self.lista_relatorio_venda.insert("", END, values=i)
        self.desconecta_bd()
        contador = 0
        ...
        total = contador
        self.ltotal_relatorio['text'] = " R${:.2f}" .format(total)
    ...
    def lista_crarrinho(self):
        self.lista_car = ttk.Treeview(self.frame_venda, height=3, column=(
            "codigo_produto", 'Nome produto', 'Quant', 'valor_produto', 'tipo'), show="headings")

        self.lista_car.heading(
            "codigo_produto", anchor='center', text="Código")

        self.lista_car.heading(
            "Nome produto", anchor='center', text="Nome")

        self.lista_car.heading(
            "valor_produto", anchor='center', text="Valor")
        self.lista_car.heading("tipo", anchor='center', text="Tipo")
        self.lista_car.heading("Quant", anchor='center', text="Quantidade")

        self.lista_car.column('codigo_produto', width=50, anchor="center")
        self.lista_car.column('Nome produto', width=50, anchor="center")
        self.lista_car.column('tipo', width=50, anchor="center")
        self.lista_car.column('Quant', width=70, anchor="center")
    ...
    # FIM DA DEFINIÇÃO DE FRAMES/ABAS.
        ...
        self.label_quant.place(relx=0.51, rely=0.40)

        self.label_tipo = Label(
            self.aba_venda_padrao, text="Tipo",  bg="#363636", fg="white", font='cambria 12')
        self.label_tipo.place(relx=0.51, rely=0.32)
        ...
        self.tipo_entry = ttk.Entry(self.aba_venda_padrao)
        ...
        self.quant.place(relx=0.51, rely=0.45, relwidth=0.20)
        self.tipo_entry.place(relx=0.51, rely=0.36, relwidth=0.20)
        ...
        self.cpf_cliente_venda.place(relx=0.51, rely=0.62, relwidth=0.20)
        self.nome_cliente_venda.place(relx=0.51, rely=0.72, relwidth=0.20)
        self.telefone_cliente_venda.place(relx=0.51, rely=0.82, relwidth=0.20)
        ...
        self.list_relatorios.place(
            relx=0.02, rely=0.18, relwidth=0.95, relheight=0.80)
        self.lista_relatorio_venda = ttk.Treeview(self.list_relatorios, height=3, column=(
            "codVenda", 'cpf', 'data', "cod", 'Nome', 'valor_item', 'Quantidade', 'valor',  'tipo'), show='headings')

        self.lista_relatorio_venda.heading(
            "codVenda", anchor='center', text="CódigoVen")
        self.lista_relatorio_venda.heading("cpf", anchor='center', text="CPF")
        self.lista_relatorio_venda.heading(
            "data", anchor='center', text="Data")
        self.lista_relatorio_venda.heading(
            "cod", anchor='center', text="Código")
        self.lista_relatorio_venda.heading(
            "Nome", anchor='center', text="Nome item")
        self.lista_relatorio_venda.heading(
            "valor_item", anchor='center', text="Valor item")
        self.lista_relatorio_venda.heading(
            "Quantidade", anchor='center', text="Quantidade")
        ...
        self.lista_relatorio_venda.column(
            'codVenda', width=60, anchor="center")
        self.lista_relatorio_venda.column('cpf', width=60, anchor="center")
        self.lista_relatorio_venda.column('data', width=60, anchor="center")
        self.lista_relatorio_venda.column('cod', width=60, anchor="center")
        self.lista_relatorio_venda.column('Nome', width=100, anchor="center")
        self.lista_relatorio_venda.column(
            'valor_item', width=60, anchor="center")
        self.lista_relatorio_venda.column(
            'Quantidade', width=60, anchor="center")
        self.lista_relatorio_venda.column('valor', width=60, anchor="center")
        self.lista_relatorio_venda.column('tipo', width=100, anchor="center")

        self.lista_relatorio_venda.place(
            relx=0.01, rely=0.02, relwidth=0.99, relheight=0.98)
        self.ldata = Label(self.aba_politica, text="Insira data Inicio:",
                           bg="#363636", fg="white", font='cambria 12')
        self.ldata.place(relx=0.02, rely=0.00)
        self.data_relatorio = Entry(self.aba_politica)
        self.data_relatorio.place(relx=0.02, rely=0.04)

        self.ldata_final = Label(self.aba_politica, text="Insira data Final:",
                                 bg="#363636", fg="white", font='cambria 12')
        self.ldata_final.place(relx=0.15, rely=0.00)

        self.data_relatorio_final = Entry(self.aba_politica)
        self.data_relatorio_final.place(relx=0.15, rely=0.04)

        self.lcodico_pesquisa = Label(self.aba_politica, text="Codigo Produto:",
                                      bg="#363636", fg="white", font='cambria 12')
        self.lcodico_pesquisa.place(relx=0.28, rely=0.00)

        self.pesquisa_cod = Entry(self.aba_politica)
        self.pesquisa_cod.place(relx=0.28, rely=0.04)

        self.ltipo = Label(self.aba_politica, text="Insira o Tipo:",
                           bg="#363636", fg="white", font='cambria 12')
        self.ltipo.place(relx=0.41, rely=0.00)

        self.pesquisa_tipo = Entry(self.aba_politica)
        self.pesquisa_tipo.place(relx=0.41, rely=0.04)

        self.lcpf = Label(self.aba_politica, text="Insira o CPF:",
                          bg="#363636", fg="white", font='cambria 12')
        self.lcpf.place(relx=0.54, rely=0.00)

        self.pesquisa_cpf = Entry(self.aba_politica)
        self.pesquisa_cpf.place(relx=0.54, rely=0.04)

        self.ltotal_relatorio = Label(self.aba_politica, text="VALOR TOTAL:",
                                      bg="#363636", fg="white", font='cambria 15')
        self.ltotal_relatorio.place(relx=0.75, rely=0.09)

        self.ltotal_relatorio = Label(self.aba_politica, text="",
                                      bg="#363636", fg="#3CB371", font='cambria 15')
        self.ltotal_relatorio.place(relx=0.88, rely=0.09)

        self.bt_confirmar_data = Button(
            self.aba_politica, text='Confirmar', bg="#3CB371", fg="white", command=self.busca_data)
        self.bt_confirmar_data.place(relx=0.02, rely=0.10, relwidth=0.09)
    ...
    def tela_usuario(self):
        ...
        self.imag = PhotoImage(file="C:\ImgProjeto\leao_login.png")
