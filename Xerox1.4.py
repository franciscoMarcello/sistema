...
class Funcs():
    ...
    def botao_duplo_click_2(self, event):
        ...
        for n in self.lista_prod.selection():
            col1, col2, col3, col4, col5, col6 = self.lista_prod.item(n, 'values')
    ...
    def altera_produto(self):
        self.variaveis_produto()
        self.conectar_bd()
        self.cursor.execute(""" UPDATE Produtos SET Nome_produto = ?,  Valor_produto = ?, data_inclusao = ?, tipo_produto = ? ,descricao_produto = ? WHERE codigo_produto = ?  """, (self.nome_produto, self.valor_produto, self.data_inclusao,self.Tipo, self.desc_produto, self.codigo_prod))
        self.conn.commit()
        self.desconecta_bd()
        self.select_lista2()
    ...
    def Selecionando_cliente(self):
        ...
        self.lcliente_selecionado["text"] = "{}"  .format(self.cpf_cliente_venda.get()) 
        self.cpf_cliente_venda.delete(0, END)
        self.nome_cliente_venda.delete(0, END)
        self.telefone_cliente_venda.delete(0,END)
    def troco_venda2(self):
        ...
        self.label_troco_resultado['text']= "R${:.2f}" .format(troco_resultado)
    def Concluir(self):
        global cpf
        global total
        self.conectar_bd()
        self.cursor.execute("INSERT INTO Vendas (CPF_cliente,Valor_da_venda) VALUES(?,?)",(cpf,total))
        self.conn.commit()
        self.desconecta_bd()
        self.troco.destroy()
        ...
        messagebox.showinfo(title="Venda concluida", message='Venda concluida com sucesso')
    ...
    def concluindo_venda(self):
        ...
        if total != 0:
            ...
            self.label_total = Label(self.troco, text='Total da venda: R${:.2f}' .format(total),font='cambria 12', bg="#363636", fg = "white")
            self.label_total.place(relx= 0.01, rely=0.01)
            self.label_troco = Label(self.troco, text='Valor recebido:',font='cambria 12', bg="#363636", fg = "white")
            self.label_troco.place(relx= 0.01, rely=0.15)
            self.valor_recebido_entry = Entry(self.troco)
            self.valor_recebido_entry.place(relx= 0.60, rely=0.17, relwidth=0.30)
            self.butao_troco = Button(self.troco, text="Troco",  bg="#3CB371", fg = "white",font='cambria 10',command=self.troco_venda2)
            self.butao_troco.place(relx=0.10, rely=0.45)
            self.butao_concluir_venda = Button(self.troco, text="Concluir",  bg="#f90f10", fg = "white",font='cambria 10',command=self.Concluir)
            self.butao_concluir_venda.place(relx=0.10, rely=0.60)
            self.troco.focus_force()
            self.troco.grab_set()
            self.label_troco_resultado = Label(self.troco, text="R$00.00",font='cambria 12', bg="#363636", fg = "white")
            ...
            self.troco.geometry("%dx%d+%d+%d"% (self.largura, self.altura, self.posx, self.posy))
   
        else:
             messagebox.showerror(title="ERRO", message="Campos vazios/Inválidos")
...
class sistema(Funcs):
    ...
    def conectar_bd(self):
        self.conn = sqlite3.connect("Xeroz.db")
    ...
    def Registro_produtos(self):
        ...
        if self.nome_produto != "" and self.valor_produto != "" and self.data_inclusao != "" and self.tipo !="" and self.desc_produto != "":
            self.conectar_bd()
            self.cursor.execute("INSERT INTO Produtos(codigo_produto, Nome_produto, Valor_produto, data_inclusao, tipo_produto, descricao_produto) VALUES(?,?,?,?,?,?)",(self.codigo_prod,self.nome_produto, self.valor_produto ,self.data_inclusao, self.Tipo,self.desc_produto))
            self.conn.commit()
            self.desconecta_bd()
            self.select_lista2()
            self.select_lista_venda_produtos()
    ...
    def select_lista_venda_produtos(self):
        self.lista_produto_venda.delete(*self.lista_produto_venda.get_children())
        self.conectar_bd()
        lista_produto_venda = self.cursor.execute(""" SELECT   codigo_produto, Nome_produto, Valor_produto, tipo_produto FROM Produtos ORDER BY codigo_produto ASC;""")
    ...
    def select_lista2(self):
        self.lista_prod.delete(*self.lista_prod.get_children())
        self.conectar_bd()
        lista2 = self.cursor.execute(""" SELECT codigo_produto, Nome_produto, Valor_produto, data_inclusao, tipo_produto, descricao_produto FROM Produtos ORDER BY codigo_produto ASC;""")
        for i in lista2:
            self.lista_prod.insert("", END, values=i)
        self.desconecta_bd()
    ...
    def select_lista_carrinho(self):
        ...
        if self.valor_produto_venda.get() != "" and self.quant.get() != "" :
            self.lista_car.insert(parent="",index='end', text=""  ,values=(self.quant.get(),self.codigo_produto_venda.get(), self.Nome_produto_venda.get(),self.valor_produto_venda.get()))
    ...
    def lista_frame2(self):
        self.lista_prod = ttk.Treeview(self.frame_2, height=3, column=("codigo_produto",'nome_produto','valor_produto', 'data_inclusao','tipo',"descricao"), show='headings')
    ...
    def lista_crarrinho(self):
        self.lista_car = ttk.Treeview(self.frame_venda, height=3,column=('Quant',"codigo_produto",'Nome produto','valor_produto'),show="headings")
        ...
        self.lista_car.heading("valor_produto", anchor='center',text="Valor")
        ...
        self.lista_car.column('valor_produto', width=45,anchor="center")
    ...
    #FIM DA DEFINIÇÃO DE FRAMES/ABAS.
        ...
        self.lb10 = Label(self.aba_cadastro_produtos, text="Cod.:",  bg="#363636", fg = "white",font='cambria 12') 
        self.lb4 = Label(self.aba_cadastro_produtos, text="Nome:",  bg="#363636", fg = "white",font='cambria 12')    
        self.lb5 = Label(self.aba_cadastro_produtos, text="Valor:", bg="#363636", fg = "white",font='cambria 12')
        ...
        self.lb_tipo = Label(self.aba_cadastro_produtos, text="Tipo: ", bg="#363636", fg = "white",font='cambria 12')
        ...
        self.descricao.place(relx=0.70, rely=0.02, relwidth=0.25, relheight=0.30)
        ...
        self.lista_produto_venda = ttk.Treeview(self.produto, height=3, column=("codigo_produto",'nome_produto','valor_produto', 'tipo'), show='headings')
        
        self.lista_produto_venda.heading("codigo_produto", anchor='center', text="Código")
        self.lista_produto_venda.heading("nome_produto", anchor='center', text="Nome")
        self.lista_produto_venda.heading("valor_produto", anchor='center',text="Valor")
        ...
        self.label_produto =Label(self.aba_venda_padrao, text="Itens",  bg="#363636", fg = "white",font='cambria 12')
        self.label_produto.place(relx=0.02 , rely=0.0)

        self.label_codigo_produto =Label(self.aba_venda_padrao, text="Codigo",  bg="#363636", fg = "white",font='cambria 12')
        self.label_codigo_produto.place(relx=0.51 , rely=0.04)

        self.label_nome_produto =Label(self.aba_venda_padrao, text="Nome",  bg="#363636", fg = "white",font='cambria 12')
        self.label_nome_produto.place(relx=0.51 , rely=0.14)

        self.label_valor_produto =Label(self.aba_venda_padrao, text="Valor",  bg="#363636", fg = "white",font='cambria 12')
        self.label_valor_produto.place(relx=0.51 , rely=0.23)

        self.label_quant =Label(self.aba_venda_padrao, text="Quantidade",  bg="#363636", fg = "white",font='cambria 12')
        ...
        self.bt_confirmar_produto_venda.place(relx=0.51, rely=0.44, relwidth=0.09) 
        ...
        self.imag_carrinho= PhotoImage(file="C:\ImgProjeto\carrinho-de-compras.png")     
        ...
        self.frame_venda.place(relx=0.73, rely=0.12, relwidth=0.26, relheight=0.70)
        ...
        self.lcliente = Label(self.aba_venda_padrao, text="CPF:" ,  bg="#363636", fg = "white", font='cambria 12')
        self.lcliente.place(relx=0.73, rely=0.83)

        self.lcliente_selecionado = Label(self.aba_venda_padrao, text='{}' .format(cpf), font='cambria 12',  bg="#363636", fg = "#fff")
        self.lcliente_selecionado.place(relx=0.77, rely=0.83)

        self.bt_confirmar_venda = Button(self.aba_venda_padrao, text='Concluir Venda', bg="#3CB371", fg="white", command=self.concluindo_venda)
